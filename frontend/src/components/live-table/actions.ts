...
// Schema for selected cells suggestions
...
// Define the schema for the array of selected cells suggestions
...
export async function generateSelectedCellsSuggestions(
  tableData: Record<string, unknown>[],
  headers: string[],
...
Please provide suggestions for each of the selected cells. Generate improvements or enhancements based on the surrounding data and context.`;
  ...
  try {
    ...
    // Basic validation: ensure we got suggestions for all selected cells
    if (response.suggestions.length !== selectedCells.length) {
      ...
      // We could match by index if lengths don't match, but this is a simpler approach
    }
